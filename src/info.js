...
Object.keys(Message.formats).forEach(function(i) {
  ...
  console.log('[' + i + '] ' + format.name + ', ' + String(Message.formatLength(format)) + ' bytes, multi-packet:', format.multipacket + ', commandable:', format.commandable);
  ...
  Object.keys(format.payload).forEach(function(j) {
    ...
    var array = payload.qty>1 ? '[' + payload.qty + ']' : '';
  	...
  	console.log('\t' + pos + '-' + (pos+Message.payloadLength(payload)-1) + ':    \t' + payload.name + array + ' (' + payload.type + ')' + conversion);
    if(payload.comment) console.log('\t\t\t  [' + payload.comment + '] ');
